...
const char *const *bip39_get_wordlist(void) {
    return wordlist;
}
...
int bip39_generate_mnemonic(char *mnemonic, size_t mnemonic_len) {
    ...
    uint8_t hash[32];
    uint8_t bits[BIP39_ENTROPY_BYTES + 1];
    int ret;
    ...
    const uint8_t *random_data = random_bytes_get(BIP39_ENTROPY_BYTES);
    memcpy(entropy, random_data, BIP39_ENTROPY_BYTES);

    ret = mbedtls_sha256(entropy, BIP39_ENTROPY_BYTES, hash, 0);
    if (ret != 0) {
        return -1;
    }

    memcpy(bits, entropy, BIP39_ENTROPY_BYTES);
    bits[BIP39_ENTROPY_BYTES] = hash[0];

    mnemonic[0] = '\0';
    for (int i = 0; i < BIP39_MNEMONIC_WORDS_24; i++) {
        int bit_offset = i * 11;
        int byte_index = bit_offset / 8;
        int bit_shift = bit_offset % 8;
        uint16_t word_index = ((uint16_t)bits[byte_index] << 8 | bits[byte_index + 1]) >> (5 - bit_shift);
        word_index &= 0x7FF;

        if (i > 0) {
            strcat(mnemonic, " ");
        }
        strcat(mnemonic, wordlist[word_index]);
    }
...
int bip39_validate_wordlist(void) {
    ...
    return (count == BIP39_WORDLIST_SIZE) ? 0 : -1;
}
